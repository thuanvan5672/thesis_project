...
@app.route("/health")
def health():
    """Kiểm tra kết nối MongoDB & Neo4j."""
    status = {}
...
def get_products():
    """
    Ví dụ: GET /mongo/products?limit=10
    Lấy dữ liệu từ collection 'products' (bạn đổi thành tên collection thật).
    """
    limit = int(request.args.get("limit", 10))
    collection_name = request.args.get("collection", "products")  # hoặc 'traicay' tùy bạn
...
def get_nodes():
    """
    Demo: MATCH (n) RETURN n LIMIT 20
    Chỉ để test kết nối.
    """
    ...
    try:
        ...
        # Neo4j driver trả object Node -> convert sang string cho đơn giản
        data = []
        for row in result:
            node = row["n"]
            data.append({
...
# --------- API NEO4J TỔNG QUÁT: CHẠY CÂU CYHPER TỪ CLIENT ---------
